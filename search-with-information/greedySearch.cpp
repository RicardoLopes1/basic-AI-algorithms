...
string greedy(int root, int destiny){
    stringstream ss;
    Grafo aux;
    int small, real = 0;
    vi ids;
    ...
    while(!ctrl){
        ...
        for(auto it : aux.getNeig()){ 
            if(!romenia_map[it.F].getVisited()){
                if(min(small, romenia_map[it.F].getSld()) < small){
                    small = romenia_map[it.F].getSld();
                    root = it.F;
                }
            }
        }// end for-each
    ...
    ss << "Calculated distance: " << real << endl;
    return ss.str();
}
...
int main(){
    ...
    cout << greedy(Arad, Bucharest) << endl;
}
